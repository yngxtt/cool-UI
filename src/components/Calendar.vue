<template>
  <div class="calendar">
    ...
    <section class="header">
      ...
      <span class="cursor" @click="handleNextMonth">▶</span>
      <span class="cursor" @click="handleYear(1)">▶▶</span>
    </section>
        ...
        <div
          ...
          <ul
            ...
            <li
              ...
              <span class="data-font calendar-item cursor-day" >{{ item.day }}</span>
            </li>
          </ul>
        </div>
      </section>
      <section v-if="isProgressBar || true"
          class="touch-area"
...
export default {
  name: 'calender',
  props:['animationTime','slideAnimation','isWeek','Height','HeightAnimation','etc','isProgressBar' ],
  data() {
    return {
      ...
      isSelectedCurrentDate: true, // 是否点选当前月份信息 (配合月视图减少点击切换时的数组更新)
      translateIndex: 0, // 轮播所在位置
      transitionDuration: this.animationTime || 0.3, // 动画持续时间                       animationTime
      needAnimation: this.slideAnimation || true, // 左右滑动是否需要动画                    slideAnimation
      isTouching: true, // 是否为touch状态
      ...
      isWeekView: this.isWeek || false, // 周视图还是月视图                          isWeek
      itemHeight: this.Height || 50, // 子元素行高                                  Height
      needHeightAnimation: this.HeightAnimation || true, // 高度变化是否需要动画               HeightAnimation
      offsetY: 0, // 周视图 Y轴偏移量
      lineNum: 0, // 当前视图总行数
      lastWeek: [], // 周视图 前一周数据
      nextWeek: [], // 周视图 后一周数据
      isDelay: this.etc || true, // 是否延迟 (动画结束在处理数据)                  etc
      touchAreaHeight: 40, // 底部区域高度
      touchAreaPadding: 10, // 底部区域padding
      isClicked: true, // 点选元素 (去除周视图切换月份时的动画延迟)
    }
  },
  created() {
